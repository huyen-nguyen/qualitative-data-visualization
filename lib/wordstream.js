        ...
        (function (data) {
            ...
            mainGroup.selectAll('.textData')
                .on('click', function (item) {
                    wordQueue[item.topic] = item.text;
                    pullDataFromTextSelection() // update new table
                    let thisText = d3.select(this);
                    let text = thisText.text();
                    let topic = thisText.attr('topic');
                    let allTexts = mainGroup.selectAll('.textData').filter(t => {
            ...
            //Build the legends
            legendGroup.attr('transform', 'translate(' + margins.left + ',' + (margins.top + legendOffset) + ')');
